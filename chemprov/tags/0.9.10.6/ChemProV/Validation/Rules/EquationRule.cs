...
namespace ChemProV.Validation.Rules
{
        ...
        public void CheckRule()
        {
            ...
            if (listOfEquations != null || listOfEquations.Count - 1 <= 0)
            {
                foreach (Tuple<object, ObservableCollection<IEquationToken>> tuple in listOfEquations)
                {
                    ...
                    ValidationResult vr = esr.CheckRule();
                    if(!vr.IsEmpty)
                    {
                ...
                {
                    ValidationResult vr = UseAllUnknowns();
                    if(!vr.IsEmpty)
                    {
                        ValidationResults.Add(vr);
                    }
    ...
    ValidationResult Solvability()
    {
                ...
                if (listOfDictionaries.Count > 0)
                {
                    ...
                    {
                        return new ValidationResult(listOfEquations[0].Item1, ErrorMessageGenerator.GenerateMesssage(ErrorMessages.Solveable));
                    }
                }
                else
                {
        ...
        float findDeterminantNbyN(float[,] matrix)
        {
            ...
            {
                //i is rows and we always through out the first 1 so start it at 1 and not 0
                int i = 1;
        ...
        {
            //i is used to keep track of where we are in the list
            int i = 0;
            float scalar = 1;
                                ...
                                try
                                {
                                    ...
                                    catch
                                    {
                                        ...
                                        if (dict.ContainsKey(equation[i]))
                                        {
                                            ...
                                            dict.Add(equation[i], currentScalar + firstValue);
                                        }
                                        else
                                        {
                                            dict.Add(equation[i], firstValue);
                                        }
                                    }
                                }
                                catch
                ...
                else
                {
                    ...
                    catch
                    {
                        ...
                        if (dict.ContainsKey(lastToken))
                        {
                            ...
                            //once it see's that i = 0 it will not bother to evaluate the other side so dont have to worry
