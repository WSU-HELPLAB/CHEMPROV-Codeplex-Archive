...
namespace ChemProV.Validation.Rules
{
    ...
    public class RuleManager
    {
        ...
        private ObservableCollection<Tuple<Object, ObservableCollection<IEquationToken>>>
            replaceUserDefinedVariables(ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>> equations,
            IList<Tuple<string, Equation>> userDefinedVariables)
        {
            foreach (Tuple<object, ObservableCollection<IEquationToken>> tuple in equations)
            {
                int i = 0;
                    ...
                    if (token is VariableToken)
                    {
                        foreach (Tuple<string, Equation> userDefinedVariable in userDefinedVariables)
                        {
                            if (token.Value == userDefinedVariable.Item1)
                            {
                                ...
                                foreach (IEquationToken item in userDefinedVariable.Item2.EquationTokens.Item2)
                                {
        ...
        /// </summary>
        private void CheckEquationSemantics(ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>> equations, IList<Tuple<string, Equation>> userDefinedVariables)
        {
            EquationRule rule = new EquationRule();

            replaceUserDefinedVariables(equations, userDefinedVariables);
        ...
        public void Validate(IEnumerable<IPfdElement> pfdElements, ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>> equations, IList<Tuple<string, Equation>> userDefinedVariables)
        {
            //clear out the dictionary before we begin adding new stuff
            EveryoneDict.Clear();
            tableDict.Clear();

            //gotta make copys cause we change them during the solving phase (i.e. subsitution)
            ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>> equationsCopy = deepCopy(equations);
            ...
            if (pfdElements != null)
            {
                ...
                CheckEquationSemantics(equationsCopy, userDefinedVariables);
            }
        }
        ...
        */
        ...
        private ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>> deepCopy(ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>> equations)
        {
            ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>> equationsDeepCopy = new ObservableCollection<Tuple<object, ObservableCollection<IEquationToken>>>();

            foreach (Tuple<object, ObservableCollection<IEquationToken>> tuple in equations)
            {
                Tuple<object, ObservableCollection<IEquationToken>> tupleDeepCopy;
