...
namespace ChemProV.Validation.Rules
{
    ...
    public class TableRule : IRule
    {
        ...
        public void CheckRule()
        {
            ...
            //Make sure target is not null and it is an IPropertiesTable
            if (target != null && target is ChemicalStreamPropertiesTable)
            {
                ITableAdapter tableAdapter = TableAdapterFactory.CreateTableAdapter(target as IPropertiesTable);

                ValidationResult vr;

                if (target is ChemicalStreamPropertiesTable)
                {
        ...
        protected ValidationResult unitsAreConsistant(ITableAdapter tableAdapter)
        {
            ...
            {
                return new ValidationResult(target, ErrorMessageGenerator.GenerateMesssage(ErrorMessages.Inconsistant_Units));
            }
        ...
        /// <summary>
        /// This is the IPropertiesTable we will be checking to see if it complies with the rule
        /// </summary>
