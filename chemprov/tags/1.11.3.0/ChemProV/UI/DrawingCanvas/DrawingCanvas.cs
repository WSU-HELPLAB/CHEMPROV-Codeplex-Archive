    ...
    public class DrawingCanvas : Canvas, IXmlSerializable
    {
        ...
        public ContextMenu NewContextMenu
        {
            ...
            set 
            {
                ...
                {
                    //this is so we cannot have multipul right click menus open.
        ...
        public void saveState(CanvasCommands command, IPfdElement selectedObject, Canvas canvas, Point lastLocation)
        {
            ...
            {
                //commented out cause it dont work
        ...
        public void DifficultySettingChanged(OptionDifficultySetting oldValue, OptionDifficultySetting newValue)
        {
            ...
            //and if we do then change em

            var uiElements = from c in this.Children
                             where c is IPfdElement
                             select c;
            List<IPfdElement> pfdElements = new List<IPfdElement>(uiElements.Cast<IPfdElement>());
            ...
            {
                //gotta add or remove temp from the tables it will figured out what one to do based on the newValue
                for (int i = 0; i < pfdElements.Count; i++)
                {
                    ...
                    if (element is IPropertiesWindow)
                    {
                        ...
                        element = PropertiesWindowFactory.TableFromTable((element as IPropertiesWindow), newValue);
        ...
        public void LoadXmlElements(XElement doc)
        {
            ...
            foreach (XElement table in tablesList.Elements())
            {
                ...
                //create the table
                IPropertiesWindow pTable = PropertiesWindowFactory.TableFromXml(table, currentDifficultySetting);
