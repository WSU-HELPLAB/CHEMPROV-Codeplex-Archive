...
namespace ChemProV
{
    ...
    /// The first (0) is the simpliest the last is the most complex
    /// </summary>
    ...
    {
        private UserDefinedVariableWindow userDefinedVariableWindow = new UserDefinedVariableWindow();
        ...
        private bool OptionDifficultySettingChanged(OptionDifficultySetting value)
        {
            ...
            try
            {
                ...
                {
                    using (IsolatedStorageFileStream isfs = new IsolatedStorageFileStream("cpv.config", FileMode.Create, isf))
                    {
                        using (StreamWriter sr = new StreamWriter(isfs))
                        {
                            sr.WriteLine(currentDifficultySetting.ToString());
                        }
                    }
                }
            }
        ...
        public MainPage()
        {
            ...
            WorkSpace.UpdateCompounds += new EventHandler(WorkSpace_UpdateCompounds);
            WorkSpace.ValidationChecked += new EventHandler(WorkSpace_ValidationChecked);
            userDefinedVariableWindow.Window.Closed += new EventHandler(userDefinedVariableWindow_Closed);
        ...
        /// <param name="e"></param>
        void MainWindow_Closing(object sender, ClosingEventArgs e)
        {
        ...
        void WorkSpace_ValidationChecked(object sender, EventArgs e)
        {
        ...
        void autoSave(object sender, EventArgs e)
        {
        ...
        {
            /////////////////////////////////////////////////////////////////////////////////
            //This commented out line is testing purposes it will remove the file so we can see what happens when nothing is there
            //or when it is corrupted
            //IsolatedStorageFile.GetUserStoreForApplication().DeleteFile("cpv.config");
            ////////////////////////////////////////////////////////////////////////////////
            ...
            using (IsolatedStorageFile isf = IsolatedStorageFile.GetUserStoreForApplication())
            {
                ...
                if (loadConfigFile)
                {
                    using (IsolatedStorageFileStream isfs = new IsolatedStorageFileStream("cpv.config", FileMode.OpenOrCreate, isf))
                    {
                        using (StreamReader sw = new StreamReader(isfs))
                        {
                            ...
                            try
                            {
                                CurrentDifficultySetting = (OptionDifficultySetting)Enum.Parse(typeof(OptionDifficultySetting), line, true);
                            }
                            catch
                            {
                                CurrentDifficultySetting = OptionDifficultySetting.MaterialBalance;
        ...
        void userDefinedVariableWindow_Closed(object sender, EventArgs e)
        {
            WorkSpace.UserDefinedVariablesUpdated(UserDefinedVaraibles);
        }

        void WorkSpace_UpdateCompounds(object sender, EventArgs e)
        {
            CompoundTable.UpdateCompounds(sender as IEnumerable<IPfdElement>);
        }

        void CompoundTable_ConstantClicked(object sender, EventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        void ToolPlaced(object sender, EventArgs e)
        {
        ...
        private void NewFileButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if(clearfile)
            {
                ...
                //also clear the variableDic in userDefinedVars
                userDefinedVariableWindow.VariableDictionary.Clear();
                userDefinedVariableWindow = new UserDefinedVariableWindow();
            }
        }
        ...
        void optionWindow_Closed(object sender, EventArgs e)
        {
