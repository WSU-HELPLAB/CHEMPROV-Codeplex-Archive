...
namespace ChemProV.UI.DrawingCanvas
{
    public class ProcessUnitUndo : SavedStateObject
    {
        ...
        public ProcessUnitUndo(CanvasCommands commandIssued, IProcessUnit ProcessUnitManipulated, Canvas CanvasUsed, Point location)
        {
            ...
            this.location = new Point(location.X + (ProcessUnitManipulated as UserControl).ActualWidth / 2, location.Y + (ProcessUnitManipulated as UserControl).ActualHeight / 2) ;

            //so we are about to remove an IProcessUnit from the canvas we gotta save his streams too.
