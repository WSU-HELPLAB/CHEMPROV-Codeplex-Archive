        ...
        public static IPropertiesTable TableFromXml(XElement tableXml)
        {
            ...
            if (objectName.CompareTo("ChemicalStreamPropertiesTable") == 0)
            {
                ...
                for (int i = 0; i < xmlData.Count(); i++ )
                {
                   ...
                   // d.Compound = 24;
                //   DataGridColumn columns = (table as ChemicalStreamPropertiesTable).PropertiesTable.Columns[3];
                    
               //    DataGridCell cellContent = columns.GetCellContent(new Tuple<string, string, string, string>(d.Label, d.Quantity, new UnitsFormatter().ConvertFromIntToString(d.Units), "Select"));
                  //  TextBlock tb = cellContent as TextBlock;
                //    tb.Text = new CompoundFormatter().ConvertFromIntToString(d.Compound);
                } 
            }
            else
            {
