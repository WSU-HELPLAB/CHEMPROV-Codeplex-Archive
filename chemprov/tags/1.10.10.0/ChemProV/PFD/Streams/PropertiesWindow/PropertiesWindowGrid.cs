...
{
    public class PropertiesWindowGrid
    {
        /// <summary>
        /// NEED TO RENAME BADLY
        /// </summary>
        private Grid my_Grid = new Grid();
        ...
        public PropertiesWindowGrid()
        {
            my_Grid.MouseLeftButtonDown += new MouseButtonEventHandler(my_Grid_MouseLeftButtonDown);
        }

        void my_Grid_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            my_Grid.CaptureMouse();
        }

        public void ClearAll()
        {
            my_Grid.Children.Clear();
            my_Grid.RowDefinitions.Clear();
            my_Grid.ColumnDefinitions.Clear();
            placeBorders();
        }

        private void changeBorderBrush()
        {
            foreach (Border br in my_Grid.Children)
            {
        ...
        private void changeBorderThickness()
        {
            foreach (Border br in my_Grid.Children)
            {
                ...
                if (hideBordersForLastRow != true || row != my_Grid.RowDefinitions.Count - 1)
                {
                    if (hideBordersForLastTwoColumns != true || column < my_Grid.ColumnDefinitions.Count - 2)
                    {
        ...
        public void UpdateGridSize()
        {
            ...
            my_Grid.SizeChanged += new SizeChangedEventHandler(PropertiesWindowGrid_FixCellBorders);
        }
        ...
        private void placeBorderAt(int column, int row)
        {
            ...
            my_Grid.Children.Add(br);
        }

        private void placeBorders()
        {
            for (int column = 0; column < my_Grid.ColumnDefinitions.Count; column++)
            {
                for (int row = 0; row < my_Grid.RowDefinitions.Count; row++)
                {
        ...
        {
            foreach (UIElement ui in my_Grid.Children)
            {
        ...
        {
            bool found = false;
            if (column >= my_Grid.ColumnDefinitions.Count)
            {
                while(my_Grid.ColumnDefinitions.Count <= column)
                {
                    my_Grid.ColumnDefinitions.Add(new ColumnDefinition());
                    int i = 0;
                    while (i < my_Grid.ColumnDefinitions.Count)
                    {
                        placeBorderAt(my_Grid.ColumnDefinitions.Count - 1, i);
                        i++;
                    }
            ...
            if (row >= my_Grid.RowDefinitions.Count)
            {
                while (my_Grid.RowDefinitions.Count <= row)
                {
                    my_Grid.RowDefinitions.Add(new RowDefinition());
                    int i = 0;
                    while (i < my_Grid.RowDefinitions.Count)
                    {
                        placeBorderAt(i, my_Grid.RowDefinitions.Count - 1);
                        i++;
                    }
                }
                placeBorders();
        ...
        {
            my_Grid.SizeChanged -= new SizeChangedEventHandler(PropertiesWindowGrid_FixCellBorders);
            foreach (UIElement child in my_Grid.Children)
            {
              ...
              //  br.Width = my_Grid.ColumnDefinitions[columnIndex].ActualWidth;
               // br.Height = my_Grid.ColumnDefinitions[rowIndex].ActualWidth;
            }
        }
    }
}
